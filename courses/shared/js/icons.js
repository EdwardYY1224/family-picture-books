...
(function () {
  ...
  const icons = {
    ...
    eye: wrap(`<path d="M9 48c10-19 25-28 39-28s29 9 39 28C77 67 62 76 48 76S19 67 9 48Z" fill="#fdfaf2"/><circle cx="48" cy="48" r="17" fill="${PALETTE.blue}"/><circle cx="48" cy="48" r="8" fill="${PALETTE.dark}"/><circle cx="43" cy="42" r="3" fill="#fff" stroke="none"/>`),
    nose: wrap(`<path d="M49 15c-3 18-8 33-12 45-2 7 3 12 11 12h9c7 0 11-5 8-10" fill="${PALETTE.skin}"/><path d="M43 75c5 5 13 5 18 0" fill="none"/>`),
    tongue: wrap(`<path d="M24 36c6-8 42-8 48 0v18c0 19-10 30-24 30S24 73 24 54Z" fill="${PALETTE.red}"/><path d="M48 55v25" fill="none" stroke="#b94f50"/>`),
    cat: wrap(`<path d="M22 31 19 10l19 14c7-3 14-3 21 0l19-14-4 22c7 7 9 17 6 28-4 16-17 25-32 25S20 76 16 60c-3-11-1-21 6-29Z" fill="${PALETTE.yellow}"/><circle cx="36" cy="50" r="4" fill="${INK}" stroke="none"/><circle cx="60" cy="50" r="4" fill="${INK}" stroke="none"/><path d="m44 60 4 3 4-3M15 59h24M57 59h24" fill="none"/>`),
    hat: wrap(`<path d="M27 56c1-24 8-37 21-37s20 13 21 37Z" fill="${PALETTE.blue}"/><path d="M12 57c13-6 59-6 72 0 4 3 2 10-4 11-23 4-41 4-64 0-6-1-8-8-4-11Z" fill="${PALETTE.blue}"/><path d="M28 48h40" fill="none" stroke="${PALETTE.yellow}" stroke-width="6"/>`),
    fish: wrap(`<path d="M18 49c13-23 39-26 58-5l13-14-1 36-13-14C56 73 31 70 18 49Z" fill="${PALETTE.blue}"/><circle cx="39" cy="43" r="4" fill="${INK}" stroke="none"/><path d="M29 56c9 5 20 5 29 0" fill="none"/>`),
    frog: wrap(`<ellipse cx="48" cy="55" rx="31" ry="26" fill="${PALETTE.green}"/><circle cx="32" cy="29" r="13" fill="${PALETTE.green}"/><circle cx="64" cy="29" r="13" fill="${PALETTE.green}"/><circle cx="32" cy="29" r="5" fill="${INK}" stroke="none"/><circle cx="64" cy="29" r="5" fill="${INK}" stroke="none"/><path d="M35 60c8 7 18 7 26 0" fill="none"/>`),
    robot: wrap(`<rect x="18" y="24" width="60" height="56" rx="15" fill="${PALETTE.blue}"/><path d="M48 24V12M39 12h18" fill="none"/><circle cx="36" cy="48" r="6" fill="${PALETTE.yellow}"/><circle cx="60" cy="48" r="6" fill="${PALETTE.yellow}"/><path d="M36 65h24M18 48H8v18M78 48h10v18" fill="none"/>`),
    phone: wrap(`<rect x="25" y="8" width="46" height="80" rx="10" fill="${PALETTE.dark}"/><rect x="30" y="17" width="36" height="57" rx="5" fill="${PALETTE.mint}"/><circle cx="48" cy="81" r="3" fill="#fff" stroke="none"/><path d="M39 42c4-8 14-8 18 0M38 56c6 6 14 6 20 0" fill="none"/>`),
    spoon: wrap(`<ellipse cx="48" cy="24" rx="14" ry="18" fill="#d9dedc"/><path d="M48 42v43" fill="none" stroke="#8d979b" stroke-width="8"/>`),
